...
# Add school logo
# urllib.request.urlretrieve("https://upload.wikimedia.org/wikipedia/en/2/2f/HRHS_logo.png", "HRHS_Logo.png")
canvas = tk.Canvas(root, width=100, height=100, highlightthickness=0, background=bg_color)
canvas.place(x=20, y=20)

image = Image.open("HRHS_Logo.png").resize((100, 100))  # load the image from wikipedia and resize it
...
# Add buttons
readme_button = tk.Button(root, text="ReadMe", command=lambda: button_click("ReadMe"), background=button_color)
readme_button.place(x=20, y=140)

partners_button = tk.Button(root, text="Existing Partners", command=lambda: button_click("Existing Partners"), background=button_color)
partners_button.place(x=20, y=186)

documentation_button = tk.Button(root, text="Documentation", command=lambda: button_click("Documentation"), background=button_color)
